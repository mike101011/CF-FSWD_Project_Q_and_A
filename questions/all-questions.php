...
if (isset($_GET["user"])) {
    ...
    if ($u_id !== sessFct()) {
        ...
        $class = "d-none";
        $questionbdy = $msg = "";
        $sql = "SELECT * FROM (tags JOIN quetag ON tags.t_id=quetag.fk_t_id JOIN questions ON quetag.fk_q_id=questions.q_id) LEFT JOIN users ON questions.fk_u_id=users.u_id WHERE u_id='$u_id'; ";
        $res = mysqli_query($connect, $sql);
        if ($res->num_rows > 0) {
            while ($row = mysqli_fetch_assoc($res)) {
                if ($row["q_resolved"] == 1) {
